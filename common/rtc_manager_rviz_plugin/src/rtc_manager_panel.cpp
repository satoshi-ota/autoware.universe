...
std::string getModuleName(const uint8_t module_type)
{
  switch (module_type) {
    ...
    case Module::AVOIDANCE_BY_LC_LEFT: {
      return "avoidance_by_lc_left";
    }
    case Module::AVOIDANCE_BY_LC_RIGHT: {
      return "avoidance_by_lc_right";
    }
...
bool isPathChangeModule(const uint8_t module_type)
{
  if (
    ...
    module_type == Module::PULL_OUT || module_type == Module::AVOIDANCE_BY_LC_LEFT ||
    module_type == Module::AVOIDANCE_BY_LC_RIGHT) {
    return true;
  }
  return false;
}
