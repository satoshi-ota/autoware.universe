...
public:
  ...
  {
    return std::make_shared<PullOutModule>(name_, *node_, module_params_, rtc_interface_);
  }
...
private:
  ...
  PullOutParameters module_params_;

  std::shared_ptr<RTCInterface> rtc_interface_;

  std::unordered_map<std::string, std::shared_ptr<PullOutModule>> registered_modules_;
};
