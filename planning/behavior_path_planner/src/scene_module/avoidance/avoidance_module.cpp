...
void AvoidanceModule::fillShiftLine(AvoidancePlanningData & data, DebugData & debug) const
{
   ...
   */
  data.new_shift_line = generateCandidateShiftLine(data.raw_shift_line, debug);
  ...
  const ShiftedPath & path, const ShiftLine & shift_line) const
{
  ...
  const auto get_start_pose = [&](const auto & ego_to_shift_start) {
    return ego_to_shift_start ? planner_data_->self_odometry->pose.pose : blinker_start_pose;
  };
